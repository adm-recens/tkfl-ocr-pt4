...
class ProductionSyncService:
    ...
    @staticmethod
    def sync_voucher_to_production(voucher_id):
        """
        Syncs a single voucher to the production 'receipts' table.
        Updates the existing receipt if it was already synced (based on ocr_voucher_id).
        """
        conn = get_connection()
        try:
            cur = conn.cursor()
            ...
            if not v:
                print(f"❌ Voucher {voucher_id} not found for sync")
                return False
            ...
            # 2. Map Deductions
            cur.execute("SELECT * FROM voucher_deductions WHERE master_id = %s", (voucher_id,))
            deductions = cur.fetchall()
            ...
            # 3. Upsert Receipt
            # Check if receipt exists for this ocr_voucher_id
            cur.execute("SELECT id FROM receipts WHERE ocr_voucher_id = %s", (voucher_id,))
            ...
            if existing_receipt:
                ...
                cur.execute("""
                    UPDATE receipts SET
                        ...
                        total_deductions = %s,
                        net_total = %s,
                        deduction_commission = %s,
                        deduction_damage = %s,
                        deduction_unloading = %s,
                        deduction_lf_cash = %s,
                        deduction_other = %s
                    WHERE id = %s
                """, (
            ...
            print(f"❌ Error syncing voucher {voucher_id}: {e}")
            conn.rollback()
            return False
